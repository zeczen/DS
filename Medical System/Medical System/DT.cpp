...
bool DT::Node::deleteSubTree(string v)
{
	...
	for (list<Answer*>::iterator i = this->_answersList.begin(); i != this->_answersList.end(); i++)
	{
		...
		if ((*i)->getNext()->deleteSubTree(v)) //if the expected noed isn't the next node and there is more nodes -> continte search.
		{
			return true; //mark that the deleting success.
		}
	}
...
DT::Answer::Answer(string ans, string next) : _answer(ans), _next(new Node(next)) ////default constructor, initalized all variables.
{
}
