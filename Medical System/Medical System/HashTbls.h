...
public:
    ...
    Item<T, K> *_at(int index);

    Item<T, K> *get(K k);

    Item<T, K> *forceGet(K k);
...
template<class T, class K>
inline Item<T, K> *HashTbls<T, K>::_at(int index) { return this->_table[index]; }
...
inline void HashTbls<T, K>::_remove(Item<T, K> *item, T t, K k) {
    ...
    if (index != -1) {
        ...
        this->_table[index]->clr(_table[index]->data, k);
    } else {
...
template<class T, class K>
Item<T, K> *HashTbls<T, K>::forceGet(K k) {
    int index = hash(k, 0);
    return this->_at(index);
}
